  ...
  <div class="container mx-auto flex flex-wrap">
    ...
    <div class="lg:w-1/2 w-full lg:px-10 lg:py-6 py-6 px-5 lg:mt-0 bg-[#5690FF]">
      ...
      <div class='grid grid-cols-2 w-fit'>
        <p class="title-font font-medium text-2xl text-gray-900 text-white mr-3">
          @money($combo["precio_total"]-($combo["precio_total"]/100)*$combo["descuento_combo"])</p>
        <p class="title-font font-medium text-1xl text-gray-900 text-white mt-auto line-through">
          @money($combo["precio_total"])</p>
      </div>

      <div class='pb-4 text-white'>
        <p class="leading-relaxed flex mt-2 items-center text-2xl text-white pb-0">Incluye</p>
        @foreach ($combo["info_contenido_combo"] as $item)

        <p>- <a class='hover:underline' href='{{route('producto_show',
            $item["producto"]->id)}}'>{{$item["producto"]->nombre_producto}}</a>
          x{{$item["cantidadCombo"]}}</p>
      ...
      <div class="flex-col">
        ...
        <p class="title-font font-medium text-2xl text-gray-900 text-white border-t-2 border-gray-200 mb-4 pt-4 text-white">
          Cantidad</p>
        <form method='POST' action='{{route('carrito_agregar', ['tipoItem'=> 'Combo', 'id' =>
          $combo["id_combo"]])}}'>
          @csrf
          <input type='number' name='unidadesCombo' min='1' max='' value='1' class='flex mr-auto mb-3 h-8 w-56 border-0 text-2xl p-0 text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'>
