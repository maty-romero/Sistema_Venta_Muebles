  ...
  <div class="container mx-auto flex flex-wrap mb-1 overflow-hidden bg-white shadow-[0_2px_4px_-0px_rgba(0,0,0,0.25)]">
    ...
    <div class="w-3/4 p-5 mt-0 bg-[#5690FF]">
      <div class="grid grid-cols-2 h-1/3">
        <a href='{{route('producto_show', $item->producto->id)}}'>
          <h1 class="text-gray-900 text-3xl title-font font-medium mb-4 text-white hover:underline hover:hover:cursor-pointer">{{$item->producto->nombre_producto}}&ensp;x{{$item->unidades}}</h1>
        </a>

        <form method='POST' action='{{route('carrito_eliminar', $item->producto->id)}}' class='ml-auto'>
          @csrf
          @method('DELETE')
      ...
      <div class="grid grid-cols-2 mt-auto h-2/3 items-end">
        ...
        <div class="align-text-bottom items-end ml-auto">
          <form method='POST' action='{{route('carrito_editar', $item->producto)}}'>
            @csrf
            @method('PATCH')
